...
\subsubsection{Clinical Data Processing}
 A pathway variant is a unique activity sequence of a complete patient trace. The ProM plug-in \texttt{Explore Event Log} organizes patient traces into pathway variants, and the total number of pathway variants is also an indicator of the level of clinical variation between patient traces. 
...
In order to reduce healthcare pathway variations to a meaningful pathway model, the pathway variants visualized by the plug-in \texttt{Explore Event Log} are examined closely to determine the most suitable processing methods. 
