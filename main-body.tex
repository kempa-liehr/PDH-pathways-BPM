...
\subsubsection{Clinical Data Processing}
 A pathway variant is a unique activity sequence of a complete patient trace. The ProM plug-in \plugin{Explore Event Log} organizes patient traces into pathway variants, and the total number of pathway variants is also an indicator of the level of clinical variation between patient traces. 
...
%One approach is to analyze length of stay at each step of the pathway variants (see section 3.3 for application example). 
%This allows for potential rate-determining steps to be identified because the contributions of all clinical activities to total length of stay is clear.
...
For the purpose of readability, Fig.~\ref{fig:appendicitis pathway
  variants} was amended by a legend.
...
\caption{Appendicitis pathway variant plot auto-generated by ProM's
  ...
  The top four variants account for approximately 88\% of the patient traces. The statistics on the left are not readable in this reproduction and are listed in Table \ref{table:appendicitis variant table}.}
...
\label{sec:ML}
The following section investigates the question if the pathway variants of the appendicitis case study are relevant exogenous variables for predicting the Length of Post-Operation Stay (LoPOS). 
This is quite a challenging task, because the individual healing process is expected to depend on personal factors like e.g. age \cite{} as well as the individual severity of the appendicitis inflamation, which in general is unknown at this stage of the data analysis, but might be captured by proxis like Length of Surgery (LOS).

For the purpose of this analysis, the patient's $LOPOS_i$, which is given as time interval between end of surgery and discharge, is converted into number of days after surgery. Therefore, a $LOPOS$ of one corresponds to a patient, who has been discharged the day after the surgery. However, we've found that 3 of the $N=448$ patient traces had a LOPOS of zero, because the surgery ended shortly after midnight and the patients were discharged on the same day. In order to simplify our analysis, the interpretation of LOPOS was broadend to /number of night rests/ after surgery, such that the LOPOS of these three patients could be casted to one.
