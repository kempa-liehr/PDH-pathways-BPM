...
\cite{BaliDwivedi2007_KM, Haas2016_KM}, which is a domain specific topic of the
broad field of knowledge management \cite{NonakaTakeuchi1995_KM, KM_befluegelt}.
This research has defined the healthcare knowledge management process,
which consists of four circular
steps\cite{Frize2007_KM}. The steps of healthcare knowledge management process are 1)
data access, 2) knowledge discovery, 3) knowledge
translation \& interpretation, as well as 4) knowledge description,
integration \& sharing.
Some examples of successful healthcare knowledge management might comprise
pediatric asthma care coordination \cite{Janevic2017_pathways},
identification of patients' specific care teams
\cite{Thornton2015_pathways}, enhancement of cancer care coordination
...
In this case study, we are combining the knowledge about patient
specific healthcare pathways of appendicitis surgeries with
probabilistic machine learning models.
For this purpose, we develop a geometric regression model, which
quantifies the recovery time of a patient in terms of a geometric
distribution, while taking into account patient specific pathway information.

The paper is structured as follows: The pathway discovery The research described in this paper investigates the utilization of
...
\caption{Basic format of example pathway variant visualized by ProM's
  plug-in \plugin{Explore Event Log}. The same colour represents the same activity, and each activity is composed of a start event and a stop event.}
...
Length of stays in hospital are analyzed based on the 13 identified appendicitis pathway variants shown in Fig.~\ref{fig:appendicitis pathway variants}. Postoperative length of stay, measured from leaving theatre to discharge, for each of the 13 appendicitis pathway variants are shown in Fig.~\ref{fig:post op length of stay appendicitis}. Pathway variant 9 has the longest postoperative length of stay. Pathway variants 5, 7 and 11 also have relatively long postoperative length of stays. Out of these four pathway variants, only pathway variant 7 consists of no postoperative activities. The number of patient traces that follow each pathway variant is limited, and more samples are required to identify rate-determining steps. Probabilistic machine learning models are used to further investigate factors influencing postoperative length of stay (Sec.~\ref{sec:ML}).
